...
bool MMControl::goFormRawPos_cb(multi_mav_manager::RawPosFormation::Request &req, multi_mav_manager::RawPosFormation::Response &res){
  ...
  ROS_WARN_STREAM("raw pos recieved " << req.goals.size() << " points");
  ...
  ROS_WARN_STREAM("raw pos should work");
...
bool MMControl::goForm(multi_mav_manager::Formation::Request &req, multi_mav_manager::Formation::Response &res)
{
  ...
  for(int i=0; i<num_active_bots; i++){
    ...
    ROS_DEBUG_STREAM("Global goal location is: " << "(" <<
        goals_[assignment_matrix_[i]](0) << ", " <<
        goals_[assignment_matrix_[i]](1) << ", " <<
        goals_[assignment_matrix_[i]](2) << ")");
    ROS_DEBUG_STREAM("Its global offset is: " << "(" <<
        active_robots_[i]->global_offset_[0] << ", " <<
        active_robots_[i]->global_offset_[1] << ", " <<
        active_robots_[i]->global_offset_[2] << ")");
    ROS_DEBUG_STREAM("So I told it: " << "(" <<
        (goals_[assignment_matrix_[i]](0) - active_robots_[i]->global_offset_[0]) << ", " <<
        (goals_[assignment_matrix_[i]](1) - active_robots_[i]->global_offset_[1]) << ", " <<
        (goals_[assignment_matrix_[i]](2) - active_robots_[i]->global_offset_[2]) << ")");

    //Goals are in global coordinates, need to transform to robot coordinates 
    srv.request.goal[0] = goals_[assignment_matrix_[i]](0) - active_robots_[i]->global_offset_[0];
    srv.request.goal[1] = goals_[assignment_matrix_[i]](1) - active_robots_[i]->global_offset_[1];
    srv.request.goal[2] = goals_[assignment_matrix_[i]](2) - active_robots_[i]->global_offset_[2];
...
void MMControl::calculateDuration(){
  ...
  t_start_ = ros::Time::now() + ros::Duration(0.5 + (num_active_bots * 0.05));  // Linearly scale start time by number of robots present
}
...
MMControl::MMControl() : nh("multi_mav_services"), priv_nh("")
{
  ...
  for(int i = 0; i < num_total_bots; i++)
  {
    ...
    std::vector<double> global_offset_vec;
    nh.getParam("/" + model_names_[i] + "/global_start_pos", global_offset_vec);
    Eigen::Vector3f global_offset;
    global_offset[0] = global_offset_vec[0];
    global_offset[1] = global_offset_vec[1];
    global_offset[2] = global_offset_vec[2];
    
    std::string str(model_names_[i]);

    std::shared_ptr<MavManagerInterface> mmi = std::make_shared<MavManagerInterface>(
          (std::string)model_names_[i], 
          (bool)act, 
          battery_low, 
          this, 
          global_offset);

    ROS_INFO_STREAM(model_names_[i] << "has global start position of (" 
      << global_offset[0] << ", "
      << global_offset[1] << ", "
      << global_offset[2] << ")");
